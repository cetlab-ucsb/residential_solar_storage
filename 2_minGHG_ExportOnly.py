...
df_params = pd.read_excel('/Users/jiajiazheng/Box/Suh\'s lab/GSRs/Jiajia/3-Residential Solar-plus-storage/'
                          'df_params_2040.xlsx', index_col=0)  # read the technical, cost and emissions parameters
...
def min_ghg(year, c_cost):
    ...
    for j in range(len(map_tmy)):
        ...
        # Objective 2 - minimize operational GHG emissions
        def obj_rule(m):
            return sum(
                m.aef[n] * m.dmd[n] * 0.001 +
                m.mef[n] * (m.e_grid_load[n] - m.dmd[n] - m.e_PV_grid[n] - m.e_batt_grid[n]) * 0.001
                for n in m.N)
        ...
        df.round(3).to_csv(
            ...
            'optimal/minGHG_ExportOnly_%(year)s/'
