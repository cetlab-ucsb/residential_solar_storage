...
def calc_lifecycle(obj, mode, year, c_cost):
    ...
    ebem_batt = unit_ebem_batt * e_max  # kgCO2e, total embodied emissions of the battery
    ...
    for i in range(len(map_tmy)):
        ...
        df['GHG_grid'] = df['AEF'] * df['Load'] * 0.001 + df['e_grid_load'] * grid_ind_ef * 0.001 + \
                         df['MEF'] * (df['e_grid_load'] - df['Load']) * 0.001  # emissions, grid
        df['GHG_PV_grid'] = - df['MEF'] * df['e_PV_grid'] * 0.001  # emissions, PV to grid
        df['GHG_batt_grid'] = - df['MEF'] * df['e_batt_grid'] * 0.001
        ...
        df['cost_grid'] = df['ToU'] * df['e_grid_load']  # hourly cost for using grid
        df['cost_feedin'] = - df['ToU'] * (df['e_PV_grid'] + df['e_batt_grid'])  # hourly feedin cost for exporting PV
        ...
        df['cost_carbon_PV+Batt'] = c_cost * 0.001 * ((df['AEF'] + grid_ind_ef) * df['e_grid_load'] * 0.001 +
        ...
        df['GHG_total_PV+Batt'] = df['GHG_grid'] + df['GHG_PV_grid'] + df['GHG_batt_grid'] + \
...
# calc_lifecycle('minCost', 'ExportOnly', 2020, 0)
calc_lifecycle('minCost', 'ExportOnly', 2020, 1e-12)
# calc_lifecycle('minCost', 'ExportOnly', 2040, 1e-12)
# calc_lifecycle('minCost', 'ExportOnly', 2020, 51)
# calc_lifecycle('minCost', 'ExportOnly', 2040, 73)
# calc_lifecycle('minGHG', 'ExportOnly', 2020, 0)
# calc_lifecycle('minGHG', 'ExportOnly', 2040, 0)
