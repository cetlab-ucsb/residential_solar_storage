...
def min_cost(year, c_cost):  # carbon cost - in $/tonneCO2
    ...
    for j in range(len(map_tmy)):
        ...
        for i in model.N:
            ...
            # battery can charge from both solar PV and the grid (different from ExportOnly mode)

        model.disc_cons = ConstraintList()
        for i in model.N:
            model.disc_cons.add(
                model.dmd[i] - (model.p_char[i] + model.p_disc[i]) * dt >= 0)
        ...
        def obj_rule(m):
            return sum(
                m.tou[n] * (m.e_grid_load[n] + m.e_grid_batt[n] - m.e_PV_grid[n]) +
                c_cost * 0.001 * (m.aef[n] * (m.e_grid_load[n] + m.e_grid_batt[n]) * 0.001)
                for n in m.N)

        model.lc_cost = Objective(rule=obj_rule, sense=1)  # Minimize total life cycle cost
        ...
        # export the data frame to the same Results folder
        df.round(3).to_csv(
            '/Users/jiajiazheng/Box/Suh\'s lab/GSRs/Jiajia/3-Residential Solar-plus-storage/Results/'
            'Optimized/minCost_ImportOnly_%(year)s_cc_%(c_cost)s_RTP/'
