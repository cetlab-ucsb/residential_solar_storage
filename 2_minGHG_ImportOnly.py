...
df_params = pd.read_excel('/Users/jiajiazheng/Box/Suh\'s lab/GSRs/Jiajia/3-Residential Solar-plus-storage/'
                          'df_params_2020.xlsx', index_col=0)  # read the technical, cost and emissions parameters
...
def min_ghg(year, c_cost):
    ...
    for j in range(len(map_tmy)):
        ...
        df = pd.read_csv('/Users/jiajiazheng/Box/Suh\'s lab/GSRs/Jiajia/3-Residential Solar-plus-storage/'
                         'Input_Data_%(year)s/%(tmy_code)s_%(utility)s_%(year)s.csv'
                         % {'year': year, 'tmy_code': tmy_code, 'utility': utility},
                         index_col=0)  # read the df containing load, PV generation, emission factors

        # calculate the energy flow under PV_only scenario as baseline
        ...
        model.e_grid_batt = Var(model.N, initialize=0, bounds=(0, p_max * dt))  # kWh, energy from grid to battery
        ...
        # model.batt_loss_cons.add(model.k[i] == (((1.0 / (eff_round ** 0.5)) + eff_round ** 0.5) / 2.0) -
        # (((1.0 / (eff_round ** 0.5)) - eff_round ** 0.5) / 2.0) * tanh(-50 * (model.p_char[i] + model.p_disc[i])))
        # Fares & Webber 2017 loss function, non-linear

        model.char_con = ConstraintList()
        for i in model.N:
            model.char_con.add(
                model.p_char[i] * dt + (model.e_PV_batt[i] + model.e_grid_batt[i]) == 0)
            # battery can charge from both solar PV and the grid

        model.disc_cons = ConstraintList()
        for i in model.N:
            model.disc_cons.add(
                model.dmd[i] - (model.p_char[i] + model.p_disc[i]) * eff_single * dt >= 0)
            # battery discharge can't exceed load at any hour
        ...
        for i in model.N:
            model.PV_cons.add(
                model.e_PV_grid[i] * (model.e_grid_load[i] + model.e_grid_batt[i]) == 0)
        ...
        for i in model.N:
            model.load_balance.add(
                model.dmd[i] == model.e_PV_load[i] + (model.p_disc[i] * dt) + model.e_grid_load[i])
        ...
        # Objective 2 - minimize total lifecycle GHG emissions
        def obj_rule(m):
            return sum(
                m.aef[n] * m.dmd[n] * 0.001 +
                m.mef[n] * (m.e_grid_load[n] + m.e_grid_batt[n] - m.dmd[n] - m.e_PV_grid[n]) * 0.001
                for n in m.N)

        model.lc_ghg = Objective(rule=obj_rule, sense=1)  # Minimize total life cycle cost

        # solve the optimization problem and print the outputs
        opt = SolverFactory("gurobi", solver_io="python")
        opt.options["NonConvex"] = 2
        # opt.options["Method"] = 0  # primal simplex, 1 - dual simplex
        # opt.options["NodefileStart"] = 0.5
        # opt.options["Threads"] = 4  # 4 processors available
        # opt.options["MIPGap"] = 0.008  # limit the gap between the lower and upper objective bound
        # opt.options["TimeLimit"] = 60.0
        ...
        df_e_grid_batt = pd.DataFrame.from_dict(model.e_grid_batt.extract_values(), orient='index',
                                                columns=[str(model.e_grid_batt)])
        ...
        df.round(3).to_csv(
            ...
            'optimal/minGHG_ImportOnly_%(year)s/'
            'optimal_minGHG_%(tmy_code)s_%(utility)s_%(year)s.csv'
            % {'tmy_code': tmy_code, 'utility': utility, 'year': year})


# run the model and record the time it takes
