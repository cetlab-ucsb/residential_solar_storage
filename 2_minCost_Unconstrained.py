...
def min_cost(year, c_cost):  # carbon cost - in $/tonneCO2
    ...
    for j in range(len(map_tmy)): #
        ...
        model.e_batt = Var(model.N, initialize=6.75, bounds=(0, e_max))  # kWh, energy stored in battery
        ...
        for i in model.N:
            ...
            # battery can charge from solar PV or grid (Unconstrained mode)
        ...
        def obj_rule(m):
            return sum(
                m.tou[n] * (m.e_grid_load[n] + m.e_grid_batt[n] - m.e_PV_grid[n] - m.e_batt_grid[n]) +
        ...
        # opt.options["MIPGap"] = 0.01  # limit the gap to <= 0.8% from the optimal solution
        ...
        df.round(3).to_csv(
            ...
            'Optimized/minCost_Unconstrained_%(year)s_cc_%(c_cost)s/'
            'optimal_minCost_%(tmy_code)s_%(utility)s_%(year)s_cc_%(c_cost)s1.csv'
